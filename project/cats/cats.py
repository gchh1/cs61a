...
def about(keywords):
    ...
    "*** YOUR CODE HERE ***"
    def check(paragraph):
        ready_paragraph = remove_punctuation(lower(paragraph))
        split_paragraph = split(ready_paragraph)
        for element in split_paragraph:
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    typed_num, source_num = len(typed_words), len(source_words)
    if source_num == 0 and typed_num == 0:
        return 100.0
    if source_num == 0 or typed_num == 0:
        return 0.0
    idx, correct = 0, 0
    for idx in range(min(typed_num, source_num)):
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        # BEGIN PROBLEM EC
        "*** YOUR CODE HERE ***"
        # 将 (typed, source) 转换为不可变结构作为缓存键
        key = deep_convert_to_tuple((typed, source))
        
        # 检查缓存是否存在该键
        if key in cache:
            cached_value, cached_limit = cache[key]
            # 当前limit小于等于缓存limit时，使用缓存结果
            if limit <= cached_limit:
                return cached_value if cached_value <= limit else limit + 1
        ...
        # 根据limit处理返回值
        return new_value if new_value <= limit else limit + 1
    return memoized
        # END PROBLEM EC
...
def furry_fixes(typed, source, limit):
    ...
    def furry_fixes_v2(t, s, lim):
        if lim < 0:
            return original_limit
        if not t and not s:
            return 0
        if len(t) != len(s):
            len_diff = abs(len(t) - len(s))
            least_length = min(len(t), len(s))
            return len_diff + furry_fixes_v2(t[:least_length], s[:least_length], lim - len_diff)
        if t[0] != s[0]:
            return 1 + furry_fixes_v2(t[1:], s[1:], lim - 1)
        return furry_fixes_v2(t[1:], s[1:], lim)
    return furry_fixes_v2(typed, source, limit)
    # END PROBLEM 6
...
def final_diff(typed, source, limit):
    ...
    for i in range(1, m+1):
        for j in range(1, n+1):
            ...
            # 常规操作
            cost = 0 if typed[i-1] == source[j-1] else 1
            
            # 检查键盘邻接关系
            if cost == 1 and typed[i-1] in KEY_ADJACENTS.get(source[j-1], []):
                cost = 0.5  # 邻接键惩罚更小
                
            dp[i][j] = min(
                dp[i-1][j] + 1,    # 删除
                dp[i][j-1] + 1,    # 插入
                dp[i-1][j-1] + cost # 替换/保持
            )
            
            # 处理重复字符（如 "oo" vs "o"）
            if i > 1 and j > 0 and typed[i-1] == source[j-1] == typed[i-2]:
                dp[i][j] = min(dp[i][j], dp[i-1][j] + 0.3)
                
            # 提前终止检查
            if dp[i][j] > limit + 1:
                return limit + 1
                
    return dp[m][n] if dp[m][n] <= limit else limit + 1


FINAL_DIFF_LIMIT = 1200  # REPLACE THIS WITH YOUR LIMIT
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == source[i]:
            ...
            break
    d = {'id': user_id, 'progress': correct / len(source)}
    upload(d)
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[sublist[i] - sublist[i - 1] for i in range(1, len(tpp[0]))] for sublist in tpp]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    player_fastest = [[] for _ in range(len(times))]
    for i in word_indices:
        ...
        for j in player_indices:
            if times[j][i] < min_time:
                min = j
                min_time = times[j][i]
        player_fastest[min].append(words[i])
