...
def boar_brawl(player_score, opponent_score):
    ...
    result = 3 * abs(opponent_tens - player_ones)
    if result:
        return result
    return 1
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    i, num, flag = 0, strategy(0, 0), True
    while i < goal:
        j = 0
        while j < goal:
            if strategy(i, j) != num:
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    idx, ans, max_aver = 1, 0, 0
    while idx <= 10:
        temp = make_averaged(roll_dice, times_called)(idx, dice)
        if temp > max_aver:
            max_aver = temp
            ans = idx
        idx += 1
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    max_aver = make_averaged(roll_dice)(6)
    if boar_brawl(score, opponent_score) >= max_aver:
        return 0
    else:
